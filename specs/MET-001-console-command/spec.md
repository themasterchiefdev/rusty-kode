...
**Canonical sequence**: 60 of 102
**Hard prerequisites**: MET-063
...
**Independent Test**: Install a release candidate in a clean supported environment, invoke `radon` with one previously delivered registered analysis subcommand using an input accepted by that subcommand, and verify that the invocation reaches the selected subcommand. Metric calculations and report contents are verified by the separate story that owns each subcommand.
...
2. **Given** the same installed product, **When** the developer invokes one previously delivered registered analysis subcommand through `radon`, **Then** the invocation reaches the subcommand named by the developer without being redirected to a different analysis subcommand.
...
| FR-003 | Acceptance Scenarios 1 and 2 verify correct selected-subcommand dispatch, using one previously delivered registered analysis subcommand. |
...
- **SC-002**: In an acceptance test using one previously delivered registered analysis subcommand, 100% of valid invocations through `radon` reach the subcommand explicitly selected by the user.
...
- The set and detailed behavior of registered analysis subcommands are defined by their owning MET stories. MET-001 verifies the installed dispatch path with one previously delivered analysis subcommand. Each subcommand-owning story remains responsible for its own registration and command-specific acceptance evidence.
